...
exports.linkEvents = ({ clockManager, clock, mhub }) => {
  ...
  // this bad boy crashes us ?
  // clock.on 'time' catchPromiseWrap.bind(null, mhub.sendTimeEvent, 'sending time event to mhub')
  clock.on('time', (time) => {
    logger.warn(`on time`)
    var result = mhub.sendTimeEvent(time)

    result.then(res => {
      logger.warn(`sendTimeEvent ACTUALLY FINISHEDP:  ${res}`)
    }).catch(err => logger.warn`sendTimeEvent ACTUALLY FAILED: ${res}`)
    
    logger.warn(`finish sendTimeEvent: ${result}`)
    return result
  })
